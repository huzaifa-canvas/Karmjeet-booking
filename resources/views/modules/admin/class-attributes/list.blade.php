...
@section('title','Class Attributes | '.config('app.name'))
@section('style')
    <style>
        ...
        <div class="content-wrapper container-xxl p-0">
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <h2 class="content-header-title float-start mb-0">Class Attributes</h2>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('admin.class-attributes.store') }}">
                                    @csrf
                                    <div class="mb-1">
                                        ...
                                        <select name="type" class="form-select" required>
                                            ...
                                            <option value="room">Room / Gym Area</option>
                                        </select>
                                    </div>
                                    <div class="mb-1">
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="card">
                            <div class="card-body pb-0">
                                ...
                                @endphp
                                <ul class="nav nav-pills attr-tabs mb-1 flex-wrap">
                                    <li class="nav-item">
                                        <a class="nav-link {{ !request('type') ? 'active' : '' }}" href="{{ route('admin.class-attributes.index') }}">
                                            All <span class="badge bg-light text-dark ms-50">{{ $totalAll }}</span>
                                        </a>
                                    </li>
                                    @foreach($typeLabels as $key => $label)
                                    <li class="nav-item">
                                        <a class="nav-link {{ request('type') == $key ? 'active' : '' }}" href="{{ route('admin.class-attributes.index', ['type' => $key]) }}">
                                            {{ $label }} <span class="badge bg-light text-dark ms-50">{{ $counts[$key] }}</span>
                                        </a>
                                    </li>
                                    @endforeach
                                </ul>
                                ...
                                {{-- Search --}}
                                <form method="GET" action="{{ route('admin.class-attributes.index') }}" class="row g-1 align-items-end mb-1">
                                    ...
                                    <div class="col-md-4 d-flex gap-1">
                                        ...
                                        <a href="{{ route('admin.class-attributes.index', request('type') ? ['type' => request('type')] : []) }}" class="btn btn-outline-secondary btn-sm"><i data-feather='x' style="width:12px;height:12px"></i></a>
                                    </div>
                                </form>
                            </div>
                                            ...
                                            <tr id="row-{{ $attr->id }}">
                                                ...
                                                <td>
                                                    ...
                                                    <span class="edit-mode-{{ $attr->id }} d-none edit-row">
                                                        <form method="POST" action="{{ route('admin.class-attributes.update', $attr->id) }}" class="d-inline">
                                                            @csrf
                                                ...
                                                <td>
                                                    <span class="badge bg-light-{{ $typeColors[$attr->type] ?? 'secondary' }}">{{ $typeLabels[$attr->type] ?? ucfirst($attr->type) }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ route('admin.class-attributes.delete', $attr->id) }}" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure?')" data-bs-toggle="tooltip" title="Delete">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="5" class="text-center py-3">
                                                    <p class="text-muted mb-0">No items found. Add an attribute using the form.</p>
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
